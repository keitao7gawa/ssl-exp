# Thank you dekkaiinu! https://github.com/dekkaiinu/hsitools/blob/main/hsitools/convert/hs_to_rgb.py
import numpy as np
import cv2
...
class HStoRGB:
    def __init__(self, lower_limit_wavelength: int=350, upper_limit_wavelength: int=1100, spectrum_stepsize: int=5, spectrum_bands: int=151):
    ...
    def __call__(self, hsi):
        ...
        rgb = extract_rgb(hsi, self.lower_limit_wavelength, self.upper_limit_wavelength, self.spectrum_stepsize, self.spectrum_bands)
        rgb = rgb.permute(2, 0, 1)
        return rgb

def hs_to_rgb(hsi: torch.Tensor, lower_limit_wavelength: int=350, upper_limit_wavelength: int=1100, spectrum_stepsize: int=5, color_matching_function: torch.Tensor = None, gamma = None):
    '''
    Parameters:
        hsi (torch.Tensor): hyperspectral image (height, width, band)
        ...
        color_matching_function (torch.Tensor): color matching function
    ...
    Returns:
        torch.Tensor: PyTorch Tensor of RGB images converted from hyperspectral images
    '''
    device = hsi.device
    ...
    if color_matching_function is None:
        color_matching_function = torch.from_numpy(get_10_deg_XYZ_CMFs()).to(device).float()  # float32に変換
        color_matching_function = color_matching_function[::spectrum_stepsize]

    wave_length = torch.arange(lower_limit_wavelength, upper_limit_wavelength + 1, spectrum_stepsize, device=device, dtype=torch.float32)
...
def gamma_correction(img: torch.Tensor, gamma: float=2.2, max_value: int=65535, base_max_value: int=255):
...
def interpolate_cmf(cmf, target_wavelengths):
    # 元の波長と値
    original_wavelengths = cmf[:, 0]
    xyz_values = cmf[:, 1:]
    
    # 補間関数の作成
    interpolator = interp1d(original_wavelengths, xyz_values, axis=0)
    
    # 新しい波長での値を計算
    interpolated_xyz = interpolator(target_wavelengths)
    
    # 新しいcolor_matching_functionの作成
    new_cmf = np.column_stack((target_wavelengths, interpolated_xyz))
    return new_cmf


def extract_rgb(hsi: torch.Tensor,
